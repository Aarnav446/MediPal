...
import { getDoctorAppointments } from '../services/db';
    ...
    <div className="max-w-7xl mx-auto px-4 py-10 sm:px-6 lg:px-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="bg-white px-4 py-2 rounded-lg shadow-sm border border-slate-200 text-sm">
            ...
            <span className="ml-2 font-bold text-slate-900">{appointments.length}</span>
        </div>
      </div>
                        ...
                        <tbody className="bg-white divide-y divide-slate-200">
                            {appointments.map((apt) => (
                                <tr key={apt.id} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-medical-600 font-medium cursor-pointer hover:underline">
                                        View Details
                                    </td>
                                </tr>
                            ))}
